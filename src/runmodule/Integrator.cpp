...
int Integrator::boundcon( float ptstate[], float err[], float& ptol ) {
	...
	// Check carbon and nitrogen state/flux variables
   	if (vegbgc) {
   		//veg C
   		for (int i=0; i<NUM_PFT_PART; i++) {
   			if (err[I_VEGC+i] > fabs( ptol * ptstate[I_VEGC+i] ) ){
   				return test = vegvarkey(I_VEGC)+1+i;
   			}

   		    if (err[I_INGPP+i] > fabs(ptol * ptstate[I_INGPP+i] )){
   		    	return test = vegvarkey(I_INGPP)+1+i;
   		    }
   		    if ( err[I_INNPP+i] > fabs(ptol * ptstate[I_INNPP+i] )){
   		    	return test = vegvarkey(I_INNPP)+1+i;
   		    }

   		    if (err[I_GPP+i] > fabs(ptol * ptstate[I_GPP+i] )){
   		    	return test = vegvarkey(I_GPP)+1+i;
   		    }
   		    if ( err[I_NPP+i] > fabs(ptol * ptstate[I_NPP+i] )){
   		    	return test = vegvarkey(I_NPP)+1+i;
   		    }

   		    if (err[I_RM+i] > fabs(ptol * ptstate[I_RM+i] )){
   		    	return test = vegvarkey(I_RM)+1+i;
   		    }
   		    if ( err[I_RG+i] > fabs(ptol * ptstate[I_RG+i] )){
   		    	return test = vegvarkey(I_RG)+1+i;
   		    }

   		    if (err[I_LTRC+i] > fabs(ptol * ptstate[I_LTRC+i] )){
   		    	return test = vegvarkey(I_LTRC)+1+i;
   		    }

   		}
   		if (err[I_DEADC] > fabs( ptol * ptstate[I_DEADC])){
   		...
   		if (veg->nfeed) {
   			for (int i=0; i<NUM_PFT_PART; i++) {
   				if (err[I_STRN+i] > fabs( ptol * ptstate[I_STRN+i] ) ){
   					return test = vegvarkey(I_STRN)+1+i;
   				}

   				if (err[I_SNUP+i] > fabs( ptol * ptstate[I_SNUP+i] )){
   					return test = vegvarkey(I_SNUP)+1+i;
   				}
   				if (err[I_NMBOL+i] > fabs( ptol * ptstate[I_NMBOL+i])){
   					return test = vegvarkey(I_NMBOL)+1+i;
   				}
   				if (err[I_NRSRB+i] > fabs( ptol * ptstate[I_NRSRB+i])){
   					return test = vegvarkey(I_NRSRB)+1+i;
   				}
   				if (err[I_LTRN+i] > fabs( ptol * ptstate[I_LTRN+i])){
   					return test = vegvarkey(I_LTRN)+1+i;
   				}

   			}

   			if (err[I_LABN] > fabs( ptol * ptstate[I_LABN] )){
   				return test = vegvarkey(I_LABN)+1;
   			}
   			if (err[I_DEADN] > fabs( ptol * ptstate[I_DEADN] )){
   				return test = vegvarkey(I_DEADN)+1;
   			}

   			if (err[I_INNUP] > fabs( ptol * ptstate[I_INNUP] )){
   				return test = vegvarkey( I_INNUP )+1;
   			}
   			if (err[I_LNUP] > fabs( ptol * ptstate[I_LNUP] )){
    ...
    //soil_bgc
   	if (soibgc) {
   		...
   		for(int il =0; il<numsl; il++){
   			...
   			if ( err[I_L_RAWC+il] > fabs( ptol * ptstate[I_L_RAWC+il] )){
   				return test = soivarkey( I_L_RAWC)+1 +il;
   			}
   			if ( err[I_L_SOMA+il] > fabs( ptol * ptstate[I_L_SOMA+il] )){
   				return test = soivarkey(I_L_SOMA)+1 +il;
   			}
   			if ( err[I_L_SOMPR+il] > fabs( ptol * ptstate[I_L_SOMPR+il] )){
   				return test = soivarkey(I_L_SOMPR) +1 + il;
   			}
   			if ( err[I_L_SOMCR+il] > fabs( ptol * ptstate[I_L_SOMCR+il] )){
   				return test = soivarkey(I_L_SOMCR)+1 + il;
   			}

   		    //
   			if (err[I_L_RH_RAW+il] > fabs( ptol * ptstate[I_L_RH_RAW+il] )){
   		    	return test = soivarkey(I_L_RH_RAW)+1+il;
   		    }
   		    if (err[I_L_RH_SOMA+il] > fabs( ptol * ptstate[I_L_RH_SOMA+il] )){
   		    	return test = soivarkey( I_L_RH_SOMA)+1+il;
   		    }
   		    if (err[I_L_RH_SOMPR+il] > fabs( ptol * ptstate[I_L_RH_SOMPR+il] )){
   		    	return test = soivarkey( I_L_RH_SOMPR)+1+il;
   		    }
   		    if (err[I_L_RH_SOMCR+il] > fabs( ptol * ptstate[I_L_RH_SOMCR+il] )){
   		    	return test = soivarkey( I_L_RH_SOMCR)+1+il;
   		    }

   		}
		if ( err[I_WDEBRIS] > fabs( ptol * ptstate[I_WDEBRIS] )){
			return test = soivarkey(I_WDEBRIS)+1;
		}
		if ( err[I_RH_WD] > fabs( ptol * ptstate[I_RH_WD] )){
   		...
   		if (ssl->nfeed) {
   			for(int il =0; il<numsl; il++){
   				if (err[I_L_ORGN+il] > fabs(ptol * ptstate[I_L_ORGN+il])){
   					return test = soivarkey(I_L_ORGN)+1+il;
   				}
   				if (err[I_L_AVLN+il] > fabs(ptol * ptstate[I_L_AVLN+il])){
   					return test = soivarkey(I_L_AVLN)+1+il;
   				}

   				//
   				if (err[I_L_NIMMOB+il] > fabs( ptol * ptstate[I_L_NIMMOB+il])){
   					return test = soivarkey(I_L_NIMMOB)+1+il;
   				}

   				if (err[I_L_NMIN+il] > fabs( ptol * ptstate[I_L_NMIN+il])){
