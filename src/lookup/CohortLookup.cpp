...
void CohortLookup::init(){
	assignVegCalpar(dircmtname);
	assignSoilCalpar(dircmtname);
...
void CohortLookup::assignVegCalpar(string & dircmt){

	string parfilecal = dircmt+"_calveg.txt";
	ifstream fctrpft;
	fctrpft.open(parfilecal.c_str(),ios::in );
	...
	for(int ip=0; ip<NUM_PFT; ip++)	fctrpft >> cmax[ip];  getline(fctrpft,str);
	for(int ip=0; ip<NUM_PFT; ip++)	fctrpft >> nmax[ip];  getline(fctrpft,str);

	for(int ip=0; ip<NUM_PFT; ip++)	fctrpft >> cfall[I_leaf][ip]; getline(fctrpft,str);
	for(int ip=0; ip<NUM_PFT; ip++)	fctrpft >> cfall[I_stem][ip]; getline(fctrpft,str);
	for(int ip=0; ip<NUM_PFT; ip++)	fctrpft >> cfall[I_root][ip]; getline(fctrpft,str);

	for(int ip=0; ip<NUM_PFT; ip++)	fctrpft >> nfall[I_leaf][ip];  getline(fctrpft,str);
	for(int ip=0; ip<NUM_PFT; ip++)	fctrpft >> nfall[I_stem][ip];  getline(fctrpft,str);
	for(int ip=0; ip<NUM_PFT; ip++)	fctrpft >> nfall[I_root][ip];  getline(fctrpft,str);

	for(int ip=0; ip<NUM_PFT; ip++)	fctrpft >> kra[ip]; getline(fctrpft,str);
	for(int ip=0; ip<NUM_PFT; ip++)	fctrpft >> krb[I_leaf][ip]; getline(fctrpft,str);
	for(int ip=0; ip<NUM_PFT; ip++)	fctrpft >> krb[I_stem][ip]; getline(fctrpft,str);
	for(int ip=0; ip<NUM_PFT; ip++)	fctrpft >> krb[I_root][ip]; getline(fctrpft,str);

	for(int ip=0; ip<NUM_PFT; ip++)	fctrpft >> frg[ip]; getline(fctrpft,str);

	fctrpft.close();

};

void CohortLookup::assignSoilCalpar(string &dircmt){

	string parfilecal = dircmt+"_calsoil.txt";
...
void CohortLookup::assignVegDimension(string &dircmt){
	...
	for(int ip=0; ip<NUM_PFT; ip++)	fctrpft >> leafmxc[ip];
	getline(fctrpft,str);     // read comments
...
void CohortLookup::assignJcmtdirname(char* jdirname){
        dircmtname = string(jdirname);
};
