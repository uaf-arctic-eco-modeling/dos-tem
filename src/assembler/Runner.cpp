...
void Runner::initInput(const string &controlfile, const string &runmode){
		...
		//region-level input
 		...
 		//initializing data inputers in TEM domains (pointers)
 		runreg.setRegionInputer(&rin);
		rungrd.setGridInputer(&gin);
...
int Runner::setupIDs(){
		...
		chtrec=runcht.cinputer.getChtDataids(runcht.cht.cd.inichtid, runcht.cht.cd.grdid,
