...
//		bd->m_soid.rhmoist[il] = getRhmoist(ed->m_soid.aws[il],  //Yuan: vwc normalized by (total pore - ice volume), which makes almost no respiration for poorly-drained BS
		...
		// soil N cycle switched on
	   	if(nfeed==1){
	   		// calculated for each layer, rather than using a mean through whole profile as previously

	   		double totc = tmp_sois.rawc[il]+tmp_sois.soma[il]
	   		...
	   		double nimmob = getNimmob(ed->m_sois.liq[il], totc,
					                  tmp_sois.orgn[il], tmp_sois.avln[il],
						              bd->m_soid.knmoist[il], bgcpar.kn2);
	   		del_soi2soi.nimmob[il] = nimmob;

	   		del_soi2soi.netnmin[il] = getNetmin(nimmob, totc, tmp_sois.orgn[il],
...
// soil inorganic N budget
void Soil_Bgc::deltaavln(){

	if (nfeed == 1){

		//total N net mineralization
		totnetnmin = 0.;
		for(int i=0;i<cd->m_soil.numsl; i++){
		...
		if (avlnflg == 1){
			...
			del_soi2l.avlnlost = 0.;
			if(totdzliq>0){     // N leaching out with drainage water
 	...
 	//ground surface wood debris decrement, if any
  	...
  	if(nfeed==1){
  	   	for(int il =0; il<cd->m_soil.numsl; il++){
  	   		...
  	   		double dondrain = 0.;   // temporarily set the DON loss with DOC
	   		...
	   		del_sois.orgn[il] -= dondrain;
  	   		...
  	   		} else { // end of avlnflg condition
  	   			del_sois.avln[il] = del_soi2soi.netnmin[il] - rtnextract[il];
