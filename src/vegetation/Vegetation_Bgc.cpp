...
void Vegetation_Bgc::deltanfeed(){
	...
	if(nfeed){
		...
		if (del_soi2v.lnuptake < 0.0 ) del_soi2v.lnuptake = 0.0;
...
double Vegetation_Bgc::getNuptake(const double & foliage, const double & raq10,
    ...
    for(int il =0; il<cd->m_soil.numsl; il++){
		bd->m_soid.knmoist[il] = getKnsoilmoist(ed->m_soid.lwc[il]);
		if (cd->m_soil.frootfrac[il][ipft]> 0.) {
			meanrzksoil += bd->m_soid.knmoist[il]*cd->m_soil.frootfrac[il][ipft];
...
double Vegetation_Bgc::getKnsoilmoist(const double & vsm){
	double ksoil;
