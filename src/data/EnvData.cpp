...
//cout << "water table" << m_soid.watertab<<"\n";	
...
void EnvData::snw_endOfDay(const int & dinm, const int & doy){
	...
	if(y_sois.snowstart<0){
	    if(d_snws.thick >0){
	      y_sois.snowstart = doy;	
	    }	
		
	}else if (y_sois.snowend<0){
		if(d_snws.thick<=0){
		  y_sois.snowend = doy;	
		}	
	}
