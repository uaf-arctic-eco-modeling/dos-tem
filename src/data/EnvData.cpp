...
void EnvData::grnd_endOfDay(const int & dinm, const int & doy){
	...
	if (m_soid.permafrost ==1){
		...
		m_soid.ald = MISSING_D;                    // NOTE: monthly 'ald' is for permafrost ONLY, but daily 'ald' for both seasonal and permafrost
		m_soid.alc = MISSING_D;                    // NOTE: monthly 'ald' is for permafrost ONLY, but daily 'ald' for both seasonal and permafrost
	}
